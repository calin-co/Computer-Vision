...
%     cuvinteVizuale - matrice k x 128, fiecare linie reprezinta un cuvant vizual (un centru al unui cluster)
  ...
  for i=1:numarImagini,
    ...
    imgcopy = rgb2gray(imread(fullfile(numeDirector,numeImagini(i).name)));
    % citeste imaginea
    ...
    % completati codul aici
    [descriptoriHOG_img, patchuri_img] = calculeazaHistogrameGradientiOrientati(img,puncteCaroiaj,dimensiuneCelula);
    descriptoriHOG = [descriptoriHOG; descriptoriHOG_img];
    patchuri = [patchuri ; patchuri_img];
  end
